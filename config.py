...
MQTT_CLIENT_ID = os.environ.get(
    "MQTT_CLIENT_ID", f"gps-no-simulation_{random.randint(0, 255):02x}{random.randint(0, 255):02x}{random.randint(0, 255):02x}")
...
REALISTIC_MOVEMENT = os.environ.get(
    "REALISTIC_MOVEMENT", "False").lower() in ('true', '1', 't')
DISTANCE_SCALING_FACTOR = float(
    os.environ.get("DISTANCE_SCALING_FACTOR", "1.0"))
REGENERATE_POSITIONS = os.environ.get(
    "REGENERATE_POSITIONS", "False").lower() in ('true', '1', 't')
MOVEMENT_SPEED = float(os.environ.get("MOVEMENT_SPEED", "10.0"))
TARGET_POINT_OFFSET_RADIUS = int(
    os.environ.get("TARGET_POINT_OFFSET_RADIUS", "200"))
